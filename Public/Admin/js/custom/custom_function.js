$(document).ready(function()
{
  ...
  $('#form').submit(function()
  {
    ...
    {
      updateAlert('必须有开始时间', 'error');
    }
    else if(endTime == '' && startTime != '')
    {
      updateAlert('必须有结束时间', 'error');
    }
    else
    {
...
function setDatePlugin(sid, eid, format_type)
{
  if(undefined == format_type)
  {
    format_type = 2;
  }
  ...
  if(eid)
  {
    $('.'+sid).datetimepicker(
    {
      ...
      startView : format_type,//点开插件后显示的界面。0、小时1、天2、月3、年4、十年，默认值2
      minView : format_type,//插件可以精确到那个时间，比如1的话就只能选择到天，不能选择小时了
      maxView : 4,//同理
      ...
      viewSelect : format_type,//默认和minView相同
