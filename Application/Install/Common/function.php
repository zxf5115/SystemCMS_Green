...
function check_dirfile(){
	$items = array(
		...
		array('file', '可写', 'success', './Application/Admin/Conf/config.php'),
...
function write_config($config, $auth){
	if(is_array($config)){
		...
		if(!IS_WRITE){
			...
			if(file_put_contents(APP_PATH . 'Admin/Conf/config.php', $conf) ){
				show_msg('配置文件写入成功');
			} else {
...
function register_administrator($db, $prefix, $admin, $auth){
	...
	$sql = "INSERT INTO `[PREFIX]member` VALUES " .
		   "('1', '[NAME]', '[PASS]', '[EMAIL]', '', '0', '0', '', '0', '1', '0', '0', '[IP]', '[TIME]', '1');";

	$password = user_md5($admin['password'], $auth);
	$sql = str_replace(
		array('[PREFIX]', '[NAME]', '[PASS]', '[EMAIL]', '[IP]', '[TIME]'),
		array($prefix, $admin['username'], $password, $admin['email'], get_client_ip(1), NOW_TIME ),
		$sql);
	//执行sql
	$db->execute($sql);
