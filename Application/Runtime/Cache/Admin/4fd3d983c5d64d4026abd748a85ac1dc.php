...
<body class="page-body">
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      <button class="btn btn-turquoise btn-add " id="action_add" url="<?php echo U('Item/add');?>">
      新 增
      </button>

      <button class="btn btn-secondary ajax-post" url="<?php echo U('Item/changeStatus',array('method'=>'resume'));?>" target-form="ids">
      启 用
      </button>

      <button class="btn btn-red ajax-post" url="<?php echo U('Item/changeStatus',array('method'=>'forbid'));?>" target-form="ids">
      禁 用
      </button>

      <button class="btn btn-blue ajax-post confirm" url="<?php echo U('Item/changeStatus',array('method'=>'delete'));?>" target-form="ids">
      删 除
      </button>
    ...
    <div class="panel-body">
      <!-- 搜索 -->
      <form id="form" class="row" action="javascript:;" url="<?php echo U('Admin/Item/index');?>" style="height:50px;">
        <div class="col-xs-2">
        ...
        <div class="col-xs-2">
          <input name="search" class="form-control" type="text" placeholder="请输入项目名称">
        </div>
      ...
      <table class="table table-bordered table-striped" id="dataTables">
        ...
        <tbody class="middle-align">
          <?php if(!empty($list)): if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="odd" role="row">
                ...
                <td>
                  <a class='btn btn-info btn-sm btn-icon icon-left' href="<?php echo U('Item/edit', array('id'=>$vo['id']));?>">编辑</a>

                  <?php if($vo["status"] == 0): ?><a url="<?php echo U('Item/changeStatus?method=resume', array('id'=>$vo['id']));?>" class='btn btn-secondary btn-sm btn-icon icon-left confirm ajax-get'>启用</a>
                  <?php else: ?>
                    <a url="<?php echo U('Item/changeStatus?method=forbid', array('id'=>$vo['id']));?>" class='btn btn-red btn-sm btn-icon icon-left confirm ajax-get '>禁用</a><?php endif; ?>

                  <a url="<?php echo U('Item/changeStatus?method=delete', array('id'=>$vo['id']));?>" class='btn btn-blue btn-sm btn-icon icon-left confirm ajax-get'>删除</a>
                </td>
