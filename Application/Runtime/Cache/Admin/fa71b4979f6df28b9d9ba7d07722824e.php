...
<div class="page-title">
  ...
  <div class="breadcrumb-env">
    <ol class="breadcrumb bc-1">
      ...
      </li><?php endif; ?> <?php if(2 == $k): ?><li class="active">
...
<div class="panel panel-default">
  <div class="panel-heading">
    <button class="btn btn-turquoise btn-add " id="action_add"
      url="<?php echo U('Role/add');?>">新 增</button>

    <button class="btn btn-secondary ajax-post"
      url="<?php echo U('Role/changeStatus',array('method'=>'resume'));?>"
      target-form="ids">启 用</button>

    <button class="btn btn-red ajax-post"
      url="<?php echo U('Role/changeStatus',array('method'=>'forbid'));?>"
      target-form="ids">禁 用</button>

    <button class="btn btn-blue ajax-post confirm"
      url="<?php echo U('Role/changeStatus',array('method'=>'delete'));?>"
  ...
  <!-- 搜索 -->
      <form id="form" class="row" action="javascript:;" url="<?php echo U('Admin/Role/index');?>" style="height:50px;">
     <div class="col-xs-2">
          <input name="roleName" value='' class="form-control" type="text" placeholder="角色名称">
        </div>
         <div class="col-xs-2">
          <input name="search" class="form-control" value="" type="text" placeholder="描述信息">
        </div>
        <div class="col-xs-2">
    ...
    <table class="table table-bordered table-striped" id="dataTables">
      ...
      <tbody class="middle-align">
        <?php if($list != ""): if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="odd" role="row">
              <td>
                <input type='checkbox' class='ids cbr' type='checkbox' name='id[]'
                  value='<?php echo ($vo["id"]); ?>'>
              </td>
              <td><?php echo ($vo["title"]); ?></td>
              <td><?php echo ($vo["description"]); ?></td>
              <td>
                <a class='skyblue' href='<?php echo U("Role/visit",array("rid"=>$vo["id"]));?>'><i class='fa fa-link'></i>&nbsp;访问授权</a>
                <a class='skyblue' href='<?php echo U("Role/user",array("rid"=>$vo["id"]));?>'><i class='fa fa-link'></i>&nbsp;成员列表</a>
              </td>
              <td><?php echo ($vo["status_format"]); ?></td>
              <td>
                <a class='btn btn-info btn-sm btn-icon icon-left' href='<?php echo U("Role/edit",array("id"=>$vo["id"]));?>'>编辑</a>

                <?php if($vo["status"] == 1): ?><a url='<?php echo U("Role/changeStatus?method=forbid",array("id"=>$vo["id"]));?>' class='btn btn-red btn-sm btn-icon icon-left  confirm ajax-get '>禁用</a>
                <?php else: ?>
                  <a url='<?php echo U("Role/changeStatus?method=resume",array("id"=>$vo["id"]));?>' class='btn btn-secondary btn-sm btn-icon icon-left confirm ajax-get'>启用</a><?php endif; ?>

                <a  url='<?php echo U("Role/changeStatus?method=delete",array("id"=>$vo["id"]));?>' class='btn btn-blue btn-sm btn-icon icon-left confirm ajax-get'>删除</a>
              </td>
...
<script src="/Public/Admin/js/common.js"></script>
  ...
  <script type="text/javascript">
    $(document).ready(function()
    {
      ...
      Think.setValue('roleName', "<?php echo ((isset($set['roleName']) && ($set['roleName'] !== ""))?($set['roleName']):'undefined'); ?>");
    });
  </script>
