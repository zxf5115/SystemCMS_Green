        ...
        <ul class="user-info-menu right-links list-inline list-unstyled">
          ...
          <li class="dropdown user-profile">
            <a href="#" data-toggle="dropdown">
              <img src="<?php echo session('user_auth.picture');?>" alt="user-image" class="img-circle img-inline userpic-32" width="30" height="30"/>
              <span>
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      <div style="float:left;">
        ...
        <button class="btn btn-info btn-add " id="action_add" url="<?php echo U('Role/user',array('rid'=> I('rid')));?>">
          成员列表
        </button>
      </div>
      <div style="float:left;margin-left:10px;">
        <select name="role" class="form-control" >
          <?php if(is_array($all_role)): $i = 0; $__LIST__ = $all_role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo U('Role/visit',array('rid'=>$vo['id']));?>" <?php if(($vo['id']) == $this_role['id']): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
      </div>
    ...
    <div class="panel-body">
      <form action="<?php echo U('Role/visit');?>" class="form-horizontal form" method="POST" role="form" >
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dl class="checkmod">
            <div class="form-group">
              <label class="col-sm-2">
                <input type="checkbox" class="default check_all" name="rules[]" value="<?php echo ($vo["id"]); ?>">&nbsp;&nbsp;
                <span><?php echo ($vo["title"]); ?></span>
              </label>
              <div class="col-sm-12">
                <div class="form-block">
                  <dd class="bd">
                    <?php if(isset($vo['child'])): if(is_array($vo['child'])): $i = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i;?><div class="check">
                          <div>
                            <label class="checkbox">
                              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                              <input name="rules[]" value="<?php echo ($child["id"]); ?>" type="checkbox" class="default check_row">
                              &nbsp;&nbsp;
                              <?php echo ($child["title"]); ?>
                            </label>
                          </div>
                          <?php if(!empty($child['operator'])): ?><span class="child_row">
                              <?php if(is_array($child['operator'])): $i = 0; $__LIST__ = $child['operator'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$op): $mod = ($i % 2 );++$i;?><div>
                                  <label class="checkbox">
                                     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                     <input name="rules[]" value="<?php echo ($op["id"]); ?>" type="checkbox" class="default">
                                    &nbsp;&nbsp;<?php echo ($op["title"]); ?>&nbsp;&nbsp;
                                  </label>
                                </div><?php endforeach; endif; else: echo "" ;endif; ?>
                            </span>
                            <span class="divsion">&nbsp;</span><?php endif; ?>
        ...
        <div class="form-group">
          <div class="hidden">
            <input type="hidden" name="id" class="form-control"  autocomplete="off" value="<?php echo ($this_role["id"]); ?>"/>
          </div>
        </div>
...
<script src="/Public/Admin/js/common.js"></script>
  ...
  <script type="text/javascript" charset="utf-8">
    ...
    {
      var rules = '<?php echo ($this_role["rules"]); ?>';
