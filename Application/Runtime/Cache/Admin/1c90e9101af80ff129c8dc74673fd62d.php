...
<body class="page-body">
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      <button class="btn btn-turquoise btn-add " id="action_add" url="<?php echo U('Label/add');?>">
      新 增
      </button>

      <button class="btn btn-secondary ajax-post" url="<?php echo U('Label/changeStatus',array('method'=>'resume'));?>" target-form="ids">
      启 用
      </button>

      <button class="btn btn-red ajax-post" url="<?php echo U('Label/changeStatus',array('method'=>'forbid'));?>" target-form="ids">
      禁 用
      </button>

      <button class="btn btn-blue ajax-post confirm" url="<?php echo U('Label/changeStatus',array('method'=>'delete'));?>" target-form="ids">
      删 除
      </button>
    ...
    <div class="panel-body">
      <!-- 搜索 -->
      <form id="form" class="row" action="javascript:;" url="<?php echo U('Admin/Label/index');?>" style="height:50px;">
        ...
        <div class="col-xs-2">
          <input name="search" class="form-control" type="text" placeholder="请输入标签名称">
        </div>
      ...
      <table class="table table-bordered table-striped" id="dataTables">
        <thead>
          <tr>
            ...
            <th class="col-xs-1">上级标签</th>
            <th class="col-xs-1">标签名称</th>
            <th class="col-xs-3">标签描述</th>
        ...
        <tbody class="middle-align">
          <?php if(!empty($list)): if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="odd" role="row">
              ...
              <td><?php echo ($vo["pid_format"]); ?></td>
              <td><?php echo ($vo["title"]); ?></td>
              <td><?php echo ($vo["description"]); ?></td>
              <td><?php echo ($vo["create_time_format"]); ?></td>
              <td><?php echo ($vo["status_format"]); ?></td>
              <td>
                <a class='btn btn-info btn-sm btn-icon icon-left' href="<?php echo U('Label/edit', array('id'=>$vo['id']));?>">编辑</a>

                <?php if($vo["status"] == 1): ?><a url='<?php echo U("Label/changeStatus?method=forbid",array("id"=>$vo["id"]));?>' class='btn btn-red btn-sm btn-icon icon-left  confirm ajax-get '>禁用</a>
                <?php else: ?>
                  <a url='<?php echo U("Label/changeStatus?method=resume",array("id"=>$vo["id"]));?>' class='btn btn-secondary btn-sm btn-icon icon-left  confirm ajax-get'>启用</a><?php endif; ?>

                <a url="<?php echo U('Label/changeStatus?method=delete', array('id'=>$vo['id']));?>" class='btn btn-blue btn-sm btn-icon icon-left confirm ajax-get'>删除</a>
              </td>
      ...
      <div class='row'>
        <div class="float_right">
          <div id="dataTables_paginate" class="dataTables_paginate paging_full_numbers">
            <ul class="pagination">
              <?php if(!empty($list)): echo ($page); endif; ?>
            </ul>
          </div>
        </div>
      </div>
...
<script src="/Public/Admin/js/common.js"></script>
  ...
  <script type="text/javascript">
    $(document).ready(function()
    {
      ...
      Think.setValue('type'      , "<?php echo ((isset($set['type']) && ($set['type'] !== ""))?($set['type']):'undefined'); ?>");
