...
<body class="page-body">
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      <button class="btn btn-turquoise btn-add " id="action_add" url="<?php echo U('User/add');?>">
      新 增
      </button>

      <button class="btn btn-secondary ajax-post" url="<?php echo U('User/changeStatus',array('method'=>'resume'));?>" target-form="ids">
      启 用
      </button>

      <button class="btn btn-red ajax-post" url="<?php echo U('User/changeStatus',array('method'=>'forbid'));?>" target-form="ids">
      禁 用
      </button>

      <button class="btn btn-blue ajax-post confirm" url="<?php echo U('User/changeStatus',array('method'=>'delete'));?>" target-form="ids">
      删 除
      </button>
    ...
    <!-- 搜索 -->
      <form id="form" class="row" action="javascript:;" url="<?php echo U('Admin/User/index');?>" style="height:50px;">
        <div class="col-xs-2">
      ...
      <table class="table table-bordered table-striped" id="dataTables">
        ...
        <tbody class="middle-align">
          <?php if($list != ""): if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="odd" role="row">
                ...
                <td><?php echo ($vo["last_login_time_format"]); ?></td>
                <td><?php echo ($vo["last_login_ip_format"]); ?></td>
                <td><?php echo ($vo["status_format"]); ?></td>
                <td>
                  <a class='btn btn-info btn-sm btn-icon icon-left' href="<?php echo U('User/edit',array('id'=>$vo['id']));?>">编辑</a>

                  <a class='btn btn-warning btn-sm btn-icon icon-left' href="<?php echo U('User/password',array('id'=>$vo['id']));?>">密码</a>

                  <a class='authorize btn btn-purple btn-sm btn-icon icon-left' href="<?php echo U('User/authorization',array('uid'=>$vo['id']));?>">授权</a>

                  <?php if($vo["status"] == 1): ?><a url='<?php echo U("User/changeStatus?method=forbid",array("id"=>$vo["id"]));?>' class='btn btn-red btn-sm btn-icon icon-left  confirm ajax-get '>禁用</a>
                  <?php else: ?>
                    <a url='<?php echo U("User/changeStatus?method=resume",array("id"=>$vo["id"]));?>' class='btn btn-secondary btn-sm btn-icon icon-left confirm ajax-get'>启用</a><?php endif; ?>

                  <a  url="<?php echo U('User/changeStatus?method=delete',array('id'=>$vo['id']));?>" class='btn btn-blue btn-sm btn-icon icon-left  confirm ajax-get'>删除</a>
                </td>
