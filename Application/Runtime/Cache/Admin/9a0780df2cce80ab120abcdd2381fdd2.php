...
<body class="page-body">
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      <button class="btn btn-turquoise btn-add " id="action_add" url="<?php echo U('Channel/add', array('pid'=>$pid));?>">
      新 增
      </button>

      <button class="btn btn-secondary ajax-post" url="<?php echo U('Channel/changeStatus',array('method'=>'resume'));?>" target-form="ids">
      启 用
      </button>

      <button class="btn btn-red ajax-post" url="<?php echo U('Channel/changeStatus',array('method'=>'forbid'));?>" target-form="ids">
      禁 用
      </button>

      <button class="btn btn-blue ajax-post confirm" url="<?php echo U('Channel/changeStatus',array('method'=>'delete'));?>" target-form="ids">
      删 除
      </button>
    ...
    <div class="panel-body">
      <!-- 搜索 -->
      <form id="form" class="row" action="javascript:;" url="<?php echo U('Admin/Channel/index');?>" style="height:50px;">
        <div class="col-xs-2">
        ...
        <div class="col-xs-2">
          <input name="search" class="form-control" type="text" placeholder="请输入SEO标题">
        </div>
      ...
      <table class="table table-bordered table-striped" id="dataTables">
        ...
        <tbody class="middle-align">
          <?php if(!empty($_list)): if(is_array($_list)): $k = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr class="odd" role="row">
                <td>
                  <input type='checkbox' class='ids cbr' type='checkbox' name='id[]' value='<?php echo ($vo["id"]); ?>'>
                </td>
                <td><?php echo ($vo["id"]); ?></td>
                <td>
                  <a class='skyblue' href="<?php echo U('Channel/index', array('pid'=>$vo['id']));?>"><i class='fa fa-link'></i><?php echo ($vo["title"]); ?></a>
                </td>
                <td><?php echo ($vo["url"]); ?></td>
                <td><?php echo ($vo["sort"]); ?></td>
                <td><?php echo ($vo["create_time"]); ?></td>
                <td>
                  <?php if($vo["status"] == 1): ?><a title="点击禁用"><span class="green">启用</span></a>
                  <?php else: ?>
                    <a title="点击启用"><span class="green">禁用</span></a><?php endif; ?>
                </td>
                <td>
                  <a class='btn btn-info btn-sm btn-icon icon-left' href="<?php echo U('Channel/edit', array('id'=>$vo['id']));?>">编辑</a>

                  <?php if($vo["status"] == 0): ?><a url="<?php echo U('Channel/changeStatus?method=resume', array('id'=>$vo['id']));?>" class='btn btn-secondary btn-sm btn-icon icon-left confirm ajax-get'>启用</a>
                  <?php else: ?>
                    <a url="<?php echo U('Channel/changeStatus?method=forbid', array('id'=>$vo['id']));?>" class='btn btn-red btn-sm btn-icon icon-left confirm ajax-get '>禁用</a><?php endif; ?>

                  <a url="<?php echo U('Channel/changeStatus?method=delete', array('id'=>$vo['id']));?>" class='btn btn-blue btn-sm btn-icon icon-left confirm ajax-get'>删除</a>
                </td>
...
<script src="/Public/Admin/js/common.js"></script>
  ...
  <script type="text/javascript">
    ...
    {
      var start_time = "<?php echo ($_set['start_time']); ?>";
      var end_time   = "<?php echo ($_set['end_time']); ?>";

      setDatePlugin('date_plugin_start', 'date_plugin_end');
      setDatePlugin('date_plugin_end');

      Think.setValue('status'    , <?php echo ((isset($_set['status']) && ($_set['status'] !== ""))?($_set['status']):"0"); ?>);
      Think.setValue('search'    , "<?php echo ((isset($_set['search']) && ($_set['search'] !== ""))?($_set['search']):'undefined'); ?>");
