...
require_once(APP_PATH . '/Admin/Common/heredoc.php');
 ...
 * 获得上次URL地址
 */
function get_before_url()
{
  $tmp    = $_SERVER['HTTP_REFERER'];
  $params = parse_url($tmp);
  return trim($params['query'], 's=');
}
...
function trimStatInfo($data = array(), $list = array())
{
  $str = '';
  $gap = '';
...
function custom_implode($data, $field)
{
  foreach($data as $vo)
  {
    $result .= $vo[$field].',';
