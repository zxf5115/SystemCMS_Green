 ...
 * @filename
 *     @chinese: 客户控制器
 *     @english: ClientController.class.php
 *
 * @version: 1.0
 * @desc   : 客户控制器
 *
...
/**
  ...
  public function index()
  {
    ...
    $result = D('Client')->getWithWhereTablePagingInfo($nowPage, $where, '*', $order);
    D('Client')->getWithWhereFormatDataInfo($result['data']);
  ...
  public function add()
  {
    if(IS_POST)
    {
      ...
      # 获取数据对象
      $result = D('Client')->doSaveFromDataAction($post);

      # 此处为自动验证错误提示信息
      if(1 == $result['flag'])
      {
        $this->success($result['msg'], U('Client/index'));
      }
      else
      {
  ...
  public function edit()
  {
    ...
    else
    {
      ...
      $where['id'] = $id;
      $list = D('Client')->getWithWhereOneTableInfo($where);
  ...
  public function changeStatus($method = null)
  {
    ...
    {
      case 'forbid':
        $this->custom_forbid('Client', $map, 'status', $url);
        break;
      case 'resume':
        $this->custom_resume('Client', $map, 'status', $url);
        break;
      case 'delete':
        $this->custom_delete('Client', $map, 'status', $url);
        break;
      default:
        $this->error('参数非法');
    }
