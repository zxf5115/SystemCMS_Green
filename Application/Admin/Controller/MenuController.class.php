 ...
 * @filename
 *     @chinese: 菜单控制器
 *     @english: MenuController.class.php
 *
 * @version: 1.0
 * @desc   : 配置菜单信息
 *
...
class MenuController extends AdminController
{
  ...
  public function index()
  {
    ...
    $result = D('Menu')->getWithWhereTablePagingInfo($nowPage, $where, '*', 'id ASC');

    $result['data'] = $this->getWithWhereFormatDataInfo($result['data'], 'Menu');
  ...
  public function add()
  {
    if(IS_POST)
    {
      ...
      # 获取数据对象
      $result = D('Menu')->doSaveFromDataAction($post);
      ...
      else if(1 == $result['flag'])
      {
        ...
        {
          # 记录行为
          record_log('add_menu', 'Menu', $result['id'], UID);
        }

        $this->success($result['msg'], U('Menu/index'));
      }
    }
    else
    {
      ...
      $menus = D('Menu')->getWithWhereTableInfo();
  ...
  public function edit($id = 0)
  {
    if(IS_POST)
    {
      ...
      else if(1 == $result['flag'])
      {
        ...
        {
          # 记录行为
          record_log('update_menu', 'Menu', $result['id'], UID);
        }
    ...
    else
    {
      ...
      /* 获取数据 */
      $info = D('Menu')->getWithWhereOneTableInfo($map);
  ...
  public function changeStatus($method = null)
  {
    ...
    {
      case 'forbid':
        $this->forbid('Menu', $map, $msg);
        break;
      case 'resume':
        $this->resume('Menu', $map, $msg);
        break;
      case 'delete':
        # 记录行为
        record_log('delete_menu', 'Menu', $id, UID);
        $this->delete('Menu', $map, $msg);
        break;
      default:
        $this->error('参数非法');
    }
