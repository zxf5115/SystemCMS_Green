...
class NoteController extends AdminController
{
  ...
  public function index()
  {
    ...
    $result = D('Note')->getWithWhereTablePagingInfo($nowPage, $where, '*', 'create_time DESC');
    ...
    D('Note')->getWithWhereFormatDataInfo($result['data']);

    # 模版赋值
    $this->assign('set', $set);
    $this->assign('list', $result['data']);
    $this->assign('page', $result['show']);
    $this->meta_title = '随笔列表';
    $this->list_title = '';
    $this->display();
  }
  ...
  public function add()
  {
    if(IS_POST)
    {
      ...
      # 获取数据对象
      $result = D('Note')->doSaveFromDataAction($post);
      ...
      if(1 == $result['flag'])
      {
        ...
        # 记录行为
        record_log('add_Note', 'Note', $result['id'], UID);

        $this->success($result['msg'], U('Note/index'));
      }
      else
      {
  ...
  public function edit()
  {
    if(IS_POST)
    {
      ...
      if(1 == $result['flag'])
      {
        ...
        # 记录行为
        record_log('edit_Note', 'Note', $post['id'], UID);
    ...
    else
    {
      ...
      $where['id'] = $id;
      $list = D('Note')->getWithWhereOneTableInfo($where);
...
// exit;
  ...
  public function changeStatus($method = null)
  {
    ...
    {
      case 'forbid':
        $this->custom_forbid('Note', $map, 'status', $url);
        break;
      case 'resume':
        $this->custom_resume('Note', $map, 'status', $url);
        break;
      case 'delete':
        # 记录行为
        record_log('delete_Note', 'Note', $id, UID);
        $this->custom_delete('Note', $map, 'status', $url);
        break;
      default:
        $this->error('参数非法');
    }
